# Conversa Documentation - Matlab
## Function Description  
### Data loading 
* `cnv_loadTrackingData` : Load tracking file 
* `cnv_loadLabelFile`: Load label file
* `cnv_applyLabels`: Align and join a tracking and label file into a data frame
* `cnv_firstChangeI`: Detect first change in behavior 
### Visualisation 
* `cnv_plot`: Plot time series and labels
### Fitting 
* `cnv_eval`: Fitting and evaluation across subjects 


## Preliminary results 
Based on the following data set with cross-valudation across subjects 

```
pid  =[1001;1005;1005;2001;2001;2002;2002;2006;2006;2024;2024];
cam  =[2;1;2;1;2;1;2;1;2;1;2;1;2];
```

For the SMILE problem The following Algorithms have 
* Null: 20% error 
* 'clinear' with {'fields',{'smile_l','smile_r'},'Learner','svm'} : 17.9% 
* 'clinear' with {'fields',allF,'Learner','svm'} :12.81%
* clinear' with  {'fields',{'smile_l','smile_r'},'Learner','logistic'}: 17.57% 
* clinear with {'fields',allF,'Learner','logistic'}: 12.23% 

For Lasso and exp(-7) is best at 12.18% 
Ridge is best at exp(-1) with at 12.57% 
